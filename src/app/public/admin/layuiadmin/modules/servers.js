 ...
 * @LastEditTime : 2020-01-23 14:23:21
 * @Description:
 */
/**
...
layui.define(['table', 'form'], function(exports) {
  ...
  table.render({
    ...
    url: '/api/servers/list?_csrf=' + csrf, //模拟接口
    method: 'GET',
    cols: [
      [
        { type: 'checkbox', fixed: 'left' },
        ...
        { field: 'ip', title: 'IP' },
        { field: 'port', width: 120, title: '端口号' },
    ...
    if (obj.event === 'del') {
      ...
      layer.confirm('确定删除此服务器？', function(index) {
        if (data.id === id) {
          ...
          admin.req({
            url: '/api/servers/del?id=' + data.id + '&_csrf=' + csrf, //实际使用请改成服务端真实接口
            method: 'delete',
            data: data,
            done: function(res) {
              layer.msg('成功删除', {
      ...
      layer.open({
        ...
        yes: function(index, layero) {
          ...
          iframeWindow.layui.form.on('submit(' + submitID + ')', function(data) {
            ...
            //提交 Ajax 成功后，静态更新表格中的数据
            admin.req({
              url: '/api/servers/edit?id=' + field.id + '&_csrf=' + csrf, //实际使用请改成服务端真实接口
              method: 'post',
              data: field,
              done: function(res) {
                if (res) {
