 ...
 * @LastEditTime : 2020-01-23 20:37:37
 * @Description:
 */
...
layui.define(['table', 'form'], function(exports) {
  ...
  table.render({
    ...
    url: '/api/pmanage/list?_csrf=' + csrf, //模拟接口
    method: 'GET',
    cols: [
      [
        { type: 'checkbox', fixed: 'left' },
        ...
        { field: 'pclassId', title: '项目类型' },
        { field: 'server', title: '所在服务器' },
        { field: 'dir', title: '项目路径' },
        { field: 'targetDir', title: '服务器部署路径' },
        // { field: 'jointime', title: '加入时间', sort: true },
  ...
  table.on('tool(LAY-user-back-manage)', function(obj) {
    ...
    if (obj.event === 'del') {
      ...
      //   layer.close(index);
      layer.confirm('确定删除此服务器？', function(index) {
        if (data.id === id) {
          ...
          admin.req({
            url: '/api/pmanage/del?id=' + data.id + '&_csrf=' + csrf, //实际使用请改成服务端真实接口
            method: 'delete',
            data: data,
            done: function(res) {
              layer.msg('成功删除', {
      ...
      admin.req({
        url: '/api/pmanage/deploy?id=' + data.id + '&_csrf=' + csrf, //实际使用请改成服务端真实接口
        method: 'post',
        data: data,
        done: function(res) {
          if (res) {
      ...
      layer.open({
        ...
        yes: function(index, layero) {
          ...
          iframeWindow.layui.form.on('submit(' + submitID + ')', function(data) {
            ...
            //提交 Ajax 成功后，静态更新表格中的数据
            admin.req({
              url: '/api/pmanage/edit?id=' + field.id + '&_csrf=' + csrf, //实际使用请改成服务端真实接口
              method: 'post',
              data: field,
              done: function(res) {
                if (res) {
